"""Part C: manual matrix calculations verified against OpenCV.

Every manual result is computed here with explicit arithmetic (NumPy indexing, loops, and
scalar math). No manual result is produced by calling the OpenCV function it is checked
against. For 3x3 neighborhood operations only the central 5x5 output is compared, so that
...
def record(
    ...
    save(source, f"{prefix}_input")
    save(manual, f"{prefix}_manual_output")
    save(opencv, f"{prefix}_opencv_output")
    difference = manual.astype(float) - opencv.astype(float)
    save(difference, f"{prefix}_difference")
    if kernel is not None:
        save(kernel, f"{prefix}_kernel")
    ...
    summaries.append(
        {
            ...
            "mean_abs_difference": round(float(np.abs(difference).mean()), 6),
            "result": "MATCH" if max_diff < 1e-9 else "MISMATCH",
        }
    )
    print(f"{prefix}: max absolute difference = {max_diff:g}")
...
def add_examples(op_id: str, title: str, patch: np.ndarray, kind: str, opencv: np.ndarray) -> None:
    ...
    for row, col in EXAMPLE_CELLS:
        ...
        if kind == "mean":
            total = sum(values)
            calculation = f"(1/9) x ({' + '.join(str(v) for v in values)}) = {total}/9 = {total / 9:.4f}"
            result = total / 9
            ...
            if kind == "gaussian":
                calculation = f"(1/16) x [{terms}] = {total}/16 = {total / 16:.4f}"
                result = total / 16
            else:
                calculation = f"{terms} = {total}"
                result = float(total)
        elif kind == "median":
            ordered = sorted(values)
            calculation = f"sorted({', '.join(str(v) for v in ordered)}) -> middle value = {ordered[4]}"
            result = float(ordered[4])
        elif kind == "erosion":
            calculation = f"min({', '.join(str(v) for v in values)}) = {min(values)}"
            result = float(min(values))
        else:
            calculation = f"max({', '.join(str(v) for v in values)}) = {max(values)}"
        ...
        worked_examples.append(
            {
                ...
                "calculation": f"sqrt(({x:g})^2 + ({y:g})^2) = sqrt({x**2 + y**2:g}) = {result:.4f}",
                "manual_result": round(result, 4),
                "opencv_result": round(float(opencv[row, col]), 4),
            }
        )
...
def verify_grayscale_pixels(image: np.ndarray) -> None:
    ...
    frame[["row", "column", "B", "G", "R"]].to_csv(
        CSV_DIR / "op01_grayscale_input.csv", index=False, header=False
    )
    save(frame[["manual_gray"]].to_numpy(), "op01_grayscale_manual_output")
    save(frame[["opencv_gray"]].to_numpy(), "op01_grayscale_opencv_output")
    save(frame[["difference"]].to_numpy(), "op01_grayscale_difference")

    max_diff = float(frame["difference"].abs().max())
    summaries.append(
        {
            "operation_id": "op01",
            ...
            "mean_abs_difference": round(float(frame["difference"].abs().mean()), 6),
            "result": "MATCH" if max_diff == 0 else "MISMATCH",
        }
    )
    print(f"op01_grayscale: {len(frame)} pixels checked, max absolute difference = {max_diff:g}")
...
def main() -> None:
    ...
    pd.DataFrame(summaries).to_csv(CSV_DIR / "verification_summary.csv", index=False)
    pd.DataFrame(worked_examples).to_csv(CSV_DIR / "manual_worked_examples.csv", index=False)
    mismatches = [s for s in summaries if s["result"] != "MATCH"]
    print(f"\n{len(summaries)} operations verified, {len(mismatches)} mismatches")
